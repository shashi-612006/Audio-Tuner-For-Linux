...
class AirTIGHT:
    ...
    def banner(self):
        ...
        print("   Status: LEAD DEVELOPER MODE   ".center(60))
        print("═" * 60)
    ...
    def run(self):
        ...
        for _ in range(5):
            ...
            alert = "⚠️" if stats["new_xruns"] > 0 else "✓"
            print(f"{stats['timestamp']:<12} | {stats['cpu']:<10}% | {stats['new_xruns']:<8} | {alert}")
            time.sleep(1)

       
        max_xruns = sum(s["new_xruns"] for s in captured_stats)
        max_cpu = max(s["cpu"] for s in captured_stats)
        status = "high_cpu" if max_cpu > 80 else "stable"
